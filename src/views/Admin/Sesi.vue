  ...
  <v-app>
          ...
          <v-row justify="center">
            <v-dialog v-model="dialog" persistent max-width="600px">
              <template v-slot:activator="{ on }">
                <v-btn icon color="primary" dark v-on="on" v-if="!btnTutup">
                  Add
    ...
    <template>
      <v-btn
        ...
        dark
        v-show="btnTutup"
        v-on:click="eliminasi()"
      >
        Tutup Sesi Terakhir</v-btn
      >
    </template>
  </v-app>
</template>
    ...
    async saveSesi() {
      try {
        ...
        obj.tgl_mulai_vote = this.formatDateOutputDatePicker(
          this.form.tgl_mulai_vote
        );
        obj.tgl_akhir_vote = this.formatDateOutputDatePicker(
          this.form.tgl_akhir_vote
        );
        obj.status_sesi = 1;
    ...
    async eliminasi() {
      if (confirm("Do you really want to close?")) {
        try {
          ...
          alert(
            "Sesi dengan ID " +
              this.idSesiVoteTutup +
              " ditutup, Jumlah Orang yang tereliminasi " +
              this.jmlTereliminasi +
              " orang "
          );
          window.location.reload();
        } catch (e) {
